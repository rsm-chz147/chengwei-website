...
Overall, this simulation shows how an A/B test can be analyzed using several core ideas from frequentist statistics. The Law of Large Numbers explains why estimates become more stable as the sample size grows, while the Central Limit Theorem justifies using an approximately Normal sampling distribution for large samples. The bootstrap provides a practical way to estimate uncertainty, and the regression version shows that the same comparison can be framed as a simple treatment-effect model. Finally, the peeking simulation highlights why researchers should decide their analysis plan before looking at results repeatedly.
